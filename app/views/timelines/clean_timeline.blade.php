<table class="table table-striped timeline-table">
	@foreach($timelines as $post)
		<tr>
			...
			<td class="timeline_quest" colspan="2"><a href="/users/{{ $post->user->region }}/{{ $post->user->summoner_name }}">{{ $post->user->summoner_name }}</a> is now friends with <a href="/users/{{ $post->friend->region }}/{{ $post->friend->summoner_name }}">{{ $post->friend->summoner_name }}</a></td>
			@elseif($post->event_type == "new_comment")
			<td class="timeline_quest" colspan="2"><a href="/users/{{ $post->user->region }}/{{ $post->user->summoner_name }}">{{ $post->user->summoner_name }}</a> has written a comment to <a href="/blogs/{{ $post->comment->blog->id }}">{{ $post->comment->blog->title }}</a></td>
